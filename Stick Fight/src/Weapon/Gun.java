...
public class Gun {
	...
	private String soundURL = "src/Sounds/ak47 shot.wav";
	private Image img;
	private String imageURL = "ak47.png";
	private int weightFactor;
	private int cy = 5;

	public Gun(Point p, int width, int height, int damage, int ammo, int weightFactor, boolean isPickedUp) {
		this.p = p;
		this.width = width;
		this.height = height;
		this.damage = damage;
	...
	public void shoot() {
		if (isPickedUp) {
			ammo--;
			Main.bullets.add(new Bullet(new Point(p.getX() + 50, p.getY() + 5), 30, 5, 75));
	...
	public void draw(Graphics g) {
		...
		if(isPickedUp) {
			g2d.rotate(rotateImageRelativeToMouse(g), p.getX() + 33, p.getY() + 16);
		}
